...
if (!isset($_GET['userId'])) {
    ...
    $conn = mysqlConnect();
    //3.ささやきデータを挿入するSQL文を実行する。
    //フォームリクエストの内容を取得
    $userId = trim(htmlspecialchars($_GET['userId'], ENT_QUOTES, 'UTF-8'));
    $imagePath = trim(htmlspecialchars($_GET['imagePath'], ENT_QUOTES, 'UTF-8'));
    $content = trim(htmlspecialchars($_GET['content'], ENT_QUOTES, 'UTF-8'));
    ...
    //echo $userId. $imagePath, $content;
}
//SQL文の作成
$sql = "INSERT INTO whisper(userId, postDate, content, imagePath) VALUES(?,CURRENT_DATE,?,?)";

//SQLステートメントを実行する準備
$stmt = mysqli_prepare($conn, $sql);

//SQLステートメントと値をバインドする
mysqli_stmt_bind_param($stmt, 'sss', $userId, $content, $imagePath);
